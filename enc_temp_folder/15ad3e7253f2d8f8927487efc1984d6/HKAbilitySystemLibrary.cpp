...
bool UHKAbilitySystemLibrary::IsSameTeam(AActor* Actor, AActor* Actor2)
{
	ICombatInterface* ActorCombat = Cast<ICombatInterface>(Actor);
	ICombatInterface* Actor2Combat = Cast<ICombatInterface>(Actor2);

	return ActorCombat->GetTeam() == Actor2Combat->GetTeam();
}
